...
fn test_galvo_15kpps() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_configs = vec![
        ...
        (
            15000,
            250,
            "15K PPS, 250 pts = 60 fps (max galvo, high refresh)",
        ),
    ...
    for (test_idx, (pps, points_per_frame, description)) in test_configs.iter().enumerate() {
        ...
        println!(
            "║ PPS: {}, Points: {}, Frame Rate: {:.1} fps                    ",
            pps,
            points_per_frame,
            *pps as f32 / *points_per_frame as f32
        );
        println!("╚═══════════════════════════════════════════════════════════════╝");
...
fn test_galvo_stress() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Drawing continuous circle at 15K PPS, {} points, {:.1} fps",
        points_per_frame,
        pps as f32 / points_per_frame as f32
    );
    ...
    for i in 0..500 {
        ...
        if let Err(e) = controller.write_frame_native(device_id, pps, HELIOS_FLAGS_DEFAULT, &frame)
        {
