...
fn test_6_detailed_timing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Expected frame time: {} µs ({:.2} ms)",
        expected_frame_time_us,
        expected_frame_time_us as f32 / 1000.0
    );
    println!(
        "  Theoretical min interval: {:.2} ms\n",
        expected_frame_time_us as f32 / 1000.0
    );
    ...
    for i in 0..1000 {
        ...
        if i > 0 && i % 100 == 0 {
            ...
            let recent_intervals: Vec<_> = actual_intervals_us
                .iter()
                .rev()
                .take(100)
                .copied()
                .collect();
            let avg_interval =
                recent_intervals.iter().sum::<u64>() as f32 / recent_intervals.len() as f32;

            println!(
                "Frame {:4}: StatusAttempts={:2} (avg={:.1}, min={}, max={}), FrameTime={:4}µs, Interval={:5.0}µs",
                i,
                status_attempts,
                avg_attempts,
                min_attempts,
    ...
    let avg_attempts =
        status_attempts_history.iter().sum::<u32>() as f32 / status_attempts_history.len() as f32;
    ...
    println!(
        "  Min: {}µs ({:.2}ms)",
        min_frame_time,
        min_frame_time as f32 / 1000.0
    );
    println!(
        "  Max: {}µs ({:.2}ms)",
        max_frame_time,
        max_frame_time as f32 / 1000.0
    );
    println!(
        "  API overhead: {:.1}%\n",
    ...
    let avg_interval =
        actual_intervals_us.iter().sum::<u64>() as f32 / actual_intervals_us.len() as f32;
    ...
    println!(
        "  Min: {}µs ({:.2}ms)",
        min_interval,
        min_interval as f32 / 1000.0
    );
    println!(
        "  Max: {}µs ({:.2}ms)",
        max_interval,
        max_interval as f32 / 1000.0
    );
    println!(
        "  Timing accuracy: {:.2}%\n",
...
fn test_6_vs_variations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (pps, points_per_frame, description) in test_configs {
        ...
        for i in 0..200 {
            ...
            if let Err(_) = controller.write_frame_native(0, pps, 0, &frame) {
                error_count += 1;
            }
        }
