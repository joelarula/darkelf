...
fn test_25fps_with_sleep() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_configs = vec![
        (
            10000,
            400,
            "10K PPS, 400 pts = 25 fps (optimal for Test 6 galvo)",
        ),
        (12500, 500, "12.5K PPS, 500 pts = 25 fps (higher detail)"),
        (
            8000,
            320,
            "8K PPS, 320 pts = 25 fps (brighter, lower speed)",
        ),
    ];
    ...
    for (test_idx, (pps, points_per_frame, description)) in test_configs.iter().enumerate() {
        ...
        println!(
            "  Point display time: {:.2}ms",
            (*points_per_frame as f32 / *pps as f32) * 1000.0
        );
        ...
        for i in 0..250 {
            ...
            if i > 0 && i % 25 == 0 {
                ...
                let recent_attempts: Vec<_> = status_attempts_history
                    .iter()
                    .rev()
                    .take(25)
                    .copied()
                    .collect();
                let avg_attempts =
                    recent_attempts.iter().sum::<u32>() as f32 / recent_attempts.len() as f32;

                println!(
                    "    Frame {:3} ({:.1}s): Actual FPS: {:.2}, Avg StatusAttempts: {:.1}",
        ...
        let avg_status_attempts = status_attempts_history.iter().sum::<u32>() as f32
            / status_attempts_history.len() as f32;
        ...
        let avg_sleep_time =
            sleep_times_us.iter().sum::<u64>() as f32 / sleep_times_us.len() as f32;
        let frames_without_sleep = sleep_times_us.iter().filter(|&&t| t == 0).count();

        println!("\n  Results:");
        println!(
            "    Total duration: {:.2}s (expected 10.0s)",
            total_duration.as_secs_f32()
        );
        println!(
            "    Actual FPS: {:.2} (target {:.1})",
            actual_fps, target_fps
        );
        println!(
            "    FPS accuracy: {:.2}%",
            (actual_fps / target_fps) * 100.0
        );
        println!(
            "    Avg status attempts: {:.2} (max: {})",
            avg_status_attempts, max_status_attempts
        );
        println!(
            "    Avg processing time: {:.0}µs ({:.2}ms)",
            avg_processing_time,
            avg_processing_time / 1000.0
        );
        println!(
            "    Max processing time: {}µs ({:.2}ms)",
            max_processing_time,
            max_processing_time as f32 / 1000.0
        );
        println!(
            "    Avg sleep time: {:.0}µs ({:.2}ms)",
            avg_sleep_time,
            avg_sleep_time / 1000.0
        );
        println!(
            "    Frames without sleep: {} ({:.1}%)",
            frames_without_sleep,
            (frames_without_sleep as f32 / 250.0) * 100.0
        );
...
fn test_25fps_variable_patterns() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (points_per_frame, description) in point_counts {
        ...
        println!(
            "    Duration: {:.2}s, FPS: {:.2}, Avg status: {:.1}, Timing errors: {}",
            total_duration.as_secs_f32(),
            actual_fps,
            avg_status,
            timing_errors
...
fn test_25fps_long_run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..total_frames {
        ...
        if i > 0 && i % 125 == 0 {
            ...
            println!(
                "  {:2}s: Frame {:4}, FPS: {:.2}, Drift: {:+}ms, Avg status: {:.1}",
                elapsed.round(),
                i,
                actual_fps,
                drift_ms,
    ...
    println!("═══════════════════════════════════════════════════════════════\n");
    println!(
        "  Total duration: {:.2}s (expected 60.0s)",
        total_duration.as_secs_f32()
    );
