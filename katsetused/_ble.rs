...
pub struct LaserController {
    ...
    ble_manager: Option<PlatformManager>,  // BLE manager
    ble_adapter: Option<Adapter>,      // BLE adapter
    ble_device: Option<Peripheral>,    // Replaced BluetoothLEDevice with Peripheral
    write_char: Option<Characteristic>, // Replaced GattCharacteristic
    notify_char: Option<Characteristic>, // Replaced GattCharacteristic
    is_ble: bool,
    ...
    connected: bool,
    mock_mode: bool,
    // Removed notification_token as btleplug uses a stream for notifications
...
impl Clone for LaserController {
    fn clone(&self) -> Self {
        LaserController {
            ...
            port: None, // SerialPort is not Clone, set to None
            ble_manager: None, // BtleManager is not Clone, set to None
            ble_adapter: None, // Adapter is not Clone, set to None
            ble_device: None, // Peripheral is not Clone, set to None
            write_char: None, // Characteristic is not Clone, set to None
            notify_char: None, // Characteristic is not Clone, set to None
            is_ble: self.is_ble,
...
impl LaserController {
    ...
    async fn connect_ble(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        let adapters = self.ble_manager.as_ref().unwrap().adapters().await?;
        self.ble_adapter = Some(adapters.into_iter().next().ok_or("No adapters found")?);

        let adapter = self.ble_adapter.as_ref().unwrap();
        adapter.start_scan(Default::default()).await?;
        tokio::time::sleep(Duration::from_secs(2)).await; // Allow time to discover devices
                    ...
                    tokio::spawn(async move {
                        while let Some(notification) = notifications.next().await {
                            if notification.uuid == characteristic_clone.uuid {
                                ...
                                println!("Notification received at 11:51 PM EEST, September 05, 2025: {}", hex);
    ...
    pub async fn send(&mut self, cmd_hex: &str, show_loading: bool, callback: Option<&mut dyn FnMut(i8, u8)>) -> Result<(), String> {
        ...
        *guard = true;
        ...
        let mut guard = self.sending.lock().unwrap();
        *guard = false;
        // Only call the callback if it hasn't already been called
        // Do not use callback after it has been moved
        if let Some(cb_ref) = cb.as_deref_mut() {
    ...
    async fn send_ble(&self, bytes: &[u8]) -> Result<(), String> {
        if let (Some(device), Some(write_char)) = (&self.ble_device, &self.write_char) {
    ...
    async fn data_received(&mut self, packet: String) {
        ...
        let should_send = if let Some(testing) = &self.device_testing {
            testing.read_uuid == "0000ffe1-0000-1000-8000-00805f9b34fb"
        } else {
            false
        };

        if should_send {
            // Update test_status before await
            ...
            // Drop mutable borrow before await
            let result = self.send(&new_cmd, false, None).await;

            // Re-borrow mutably to update after await
            if let Some(testing) = &mut self.device_testing {
                if let Err(e) = result {
    ...
    pub async fn start_test(&mut self, test: &mut DeviceTesting, max_retries: i32) {
        ...
        if let Some(testing) = self.device_testing.as_mut() {
            if testing.can_send {
                ...
                Box::pin(self.start_test(test, max_retries - 1)).await;
            }
        }
    }
        ...
        if let Err(e) = result {
            ...
            tokio::spawn(async move {
                ...
                if let Some(testing) = &mut controller.device_testing {
                    // Removed: testing.rec_device_msg_timer = None;
                    if testing.conn {
                        testing.test_status = "Test ended".to_string();
                        testing.test_result = "Not passed".to_string();
    ...
    pub async fn close_cnn_and_run(&mut self, callback: Option<Box<dyn Fn()>>) {
        if let Some(device) = self.ble_device.take() {
            device.disconnect().await.ok(); // Close BLE connection
        }
        if let Some(adapter) = self.ble_adapter.as_ref() {
            adapter.stop_scan().await.ok(); // Stop scanning
        }
    ...
    pub async fn do_send_data(&mut self, test: &DeviceTesting, chunks: &[Vec<u8>], index: usize, callback: Option<Box<dyn Fn(String)>>) {
        ...
        if let Err(e) = result {
            ...
            tokio::time::sleep(Duration::from_millis(20)).await;
            Box::pin(self.do_send_data(test, chunks, index + 1, callback)).await;
    ...
    pub async fn close_bluetooth_adapter(&mut self) {
        ...
        self.ble_manager = None; // Release manager
    }
}
